...
//// DONE: fill this function to realize plane mapping
void calcPlaneMapping(void)
{
    ...
    for (int i = 0; i < myObject.vertices.size(); i++) {
        Point coords = myObject.vertices[i].v;
        float x = coords[0];
        float y = coords[1];

        float u = (x - xmin) / (xmax - xmin);
        float v = (y - ymin) / (ymax - ymin);
        myObject.vertices[i].t[0] = u;
        myObject.vertices[i].t[1] = v;
    }
}
...
{ 
    float zmin = myObject.bBox.minP[2];
    float zmax = myObject.bBox.maxP[2];
    ...
    for (int i = 0; i < myObject.vertices.size(); i++) {
        ...
        float theta = (atan2f(y, x) + PI) / (PI * 2);
        float phi = (z - zmin) / (zmax - zmin);
        myObject.vertices[i].t[0] = theta;
        myObject.vertices[i].t[1] = phi;
    }
}


//// DONE: fill this function to realize sphere mapping
void calcSphereMapping(void)
{
    for (int i = 0; i < myObject.vertices.size(); i++) {
        ...
        float theta = (atan2f(y, x) + PI) / (PI * 2);
        float temp = sqrtf(pow(x, 2) + pow(y, 2));
        float phi = atan2f(temp, z) / PI;
        myObject.vertices[i].t[0] = theta;
        myObject.vertices[i].t[1] = phi;
    }
}
